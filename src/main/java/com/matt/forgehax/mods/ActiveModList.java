...
public class ActiveModList extends HudMod {
  ...
  private final Setting<Boolean> minimize =
		  getCommandStub()
		  	.builders()
		  	.<Boolean>newSettingBuilder()
		  	.name("minimize")
		  	.description("Minimize the mod list")
		  	.defaultTo(true)
		  	.build();
  
  private final Setting<Boolean> showStamp =
		  getCommandStub()
		  	.builders()
		  	.<Boolean>newSettingBuilder()
		  	.name("showStamp")
		  	.description("Shows \"ForgeHax\"")
		  	.defaultTo(true)
		  	.build();
  ...
  public void onRenderScreen(RenderGameOverlayEvent.Text event) {
    ...
    if(showStamp.getAsBoolean()) {
    	SurfaceHelper.drawRainbowTextAlign("ForgeHax", getPosX(0), getPosY(2), 
        		scale.get() * 1.25, true, align);
        
        SurfaceHelper.drawRainbowTextAlign("TheAlphaEpsilon Ver.", getPosX(0), getPosY(15), 
        		scale.get(), true, align);
    }
    ...
    if (MC.currentScreen instanceof GuiChat || MC.gameSettings.showDebugInfo || minimize.getAsBoolean()) {
    ...
    SurfaceHelper.drawTextAlign(text, getPosX(0), getPosY(30),
