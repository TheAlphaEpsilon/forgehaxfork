...
public class ForgeHaxHooks implements ASMCommon {
  ...
  public static final HookReporter HOOK_onPlayerTravel =
	newHookReporter()
		  .hook("onPlayerTravel")
		  .dependsOn(TypesMc.Methods.EntityPlayer_travel)
		  .forgeEvent(PlayerTravelEvent.class)
		  .build();
  
  public static boolean onPlayerTravel() {
	  return HOOK_onPlayerTravel.reportHook()
			  && MinecraftForge.EVENT_BUS.post(new PlayerTravelEvent());
  }
